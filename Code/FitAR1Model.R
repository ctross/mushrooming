...
for(j in 1:MaxTrip){
{
 ...
 for(i in (Lags+1):N[j]){
  PredDist[i-Lags] = AlphaDist + AR_Dist*Distance[i-1,j];
     for(k in 1:Lags){
    PredDist[i-Lags] = PredDist[i-Lags] + BetaDist[k]*Yield[i-k,j];
        }}

  for(i in (Lags+1):N[j]){
  PredAngle[i-Lags] = AlphaAngle + AR_Angle*AngDiff[i-1,j];
     for(k in 1:Lags){
...
'
 ...
 m2 <- stan( model_code=model_code_2, data=model_dat,refresh=1,chains=2,control=list(max_treedepth=13))
