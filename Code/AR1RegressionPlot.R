...
m2a<-rstan::extract(m2,pars="BetaAngle")
sample_eff<-apply(m2a$BetaAngle,2,quantile,probs=c(0.05,0.5,0.95))
df_angle<-data.frame(Lags=c(1:Lags),Group="Turning-Angle",
                      LI=sample_eff[1,],
                      Median=sample_eff[2,],
                      HI=sample_eff[3,])
                      
m2d<-rstan::extract(m2,pars="BetaDist")
sample_eff<-apply(m2d$BetaDist,2,quantile,probs=c(0.05,0.5,0.95))
...
CairoPDF("CaterAngleAR.pdf",width=4.5,height=4.5)
print(g1)
dev.off()

CairoPDF("CaterStepAR.pdf",width=4.5,height=4.5)
print(g2)
dev.off()
